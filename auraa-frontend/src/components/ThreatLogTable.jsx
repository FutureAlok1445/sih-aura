...
export default function ThreatLogTable({ attacks = [], onSearch, selectedSeverity = "All", onSeverityChange, onSelectAttack }) {
    ...
    const getSeverityBadge = (severityRaw) => {
        ...
        if (severity === "critical") colorClass = "bg-red-500/10 text-red-500 border-red-500/20";
        else if (severity === "high") colorClass = "bg-orange-500/10 text-orange-500 border-orange-500/20";
        else if (severity === "medium") colorClass = "bg-yellow-500/10 text-yellow-500 border-yellow-500/20";
    ...
    // Updated Export Logic with new columns
    const exportCSV = () => {
        const headers = [
            "Timestamp", "Source IP", "Dest IP", "Method", "URL", 
            "Post Body", "Status Code", "Decision", "Severity"
        ];
        const csvContent = [
            headers.join(","),
            ...filteredData.map((row) => {
                const sc = parseStatusCode(row.status_code);
                const decision = sc >= 200 && sc < 300 ? "BREACH" : sc === 0 ? "UNKNOWN" : "BLOCKED";
                const body = (row.post_body || "").replace(/"/g, '""').replace(/\n/g, " "); // Clean body
                return [
                    row.timestamp, row.ip, row.dest_ip, row.method, 
                    `"${row.url}"`, `"${body}"`, row.status_code, decision, row.severity
                ].join(",");
            })
        ].join("\n");
        ...
        <div className="w-full space-y-4 p-6 border border-white/10 rounded-xl bg-black/40 backdrop-blur-md shadow-sm">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Real-time analysis of HTTP traffic.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <div className="relative z-10">
                        <FluidDropdown selectedSeverity={selectedSeverity} onSeverityChange={onSeverityChange} />
                    </div>
                    <div className="relative">
                        ...
                        {isExportOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-black/90 border border-white/10 rounded-lg shadow-xl z-50 py-1">
                                <button onClick={() => { exportCSV(); setIsExportOpen(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-white/10">
                                    <FileSpreadsheet className="w-4 h-4 text-green-400" /> CSV
                                </button>
                                <button onClick={() => { exportJSON(); setIsExportOpen(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-white/10">
                ...
                <table className="w-full text-sm text-left">
                    <thead className="text-xs text-gray-400 uppercase bg-white/5">
                        <tr className="border-b border-white/10">
                            {/* Group Headers */}
                            <th colSpan="4" className="px-6 py-2 text-left font-bold text-blue-400 text-xs border-r border-white/5">
                                1. INGEST STREAM
                            </th>
                            <th colSpan="2" className="px-6 py-2 text-left font-bold text-yellow-400 text-xs border-r border-white/5">
                                2. PAYLOAD ANALYSIS
                            </th>
                            <th colSpan="2" className="px-6 py-2 text-left font-bold text-red-400 text-xs">
                                3. DECISION
                            </th>
                        </tr>
                        <tr>
                    ...
                    <tbody className="divide-y divide-white/10">
                        {paginatedData.length > 0 ? (
                            paginatedData.map((attack, index) => (
                                <tr 
                                    key={index} 
                                    onClick={() => onSelectAttack && onSelectAttack(attack)}
                                    className="hover:bg-blue-500/10 cursor-pointer transition-colors border-b border-white/5"
                                >
                                    {/* 1. INGEST */}
                                    <td className="px-6 py-3 text-gray-400 font-mono text-[11px] whitespace-nowrap">
                                        {attack.timestamp ? new Date(attack.timestamp).toLocaleTimeString() : "-"}
                                    </td>
                                    <td className="px-6 py-3 font-mono text-xs text-blue-300">
                                        {attack.ip}
                                    </td>
                                    <td className="px-6 py-3 font-mono text-xs text-purple-300">
                                        {attack.dest_ip || "-"}
                                    </td>
                                    <td className="px-6 py-3 font-mono text-xs">
                                        <span className={`px-1.5 py-0.5 rounded ${attack.method === 'POST' ? 'bg-orange-500/20 text-orange-400' : 'bg-gray-700 text-gray-300'}`}>
                                            {attack.method || "GET"}
                                        </span>
                                    </td>

                                    {/* 2. PAYLOAD */}
                                    <td className="px-6 py-3 text-gray-300 max-w-[200px] truncate" title={attack.url}>
                                        {attack.url}
                                    </td>
                                        ...
                                        {attack.post_body && attack.post_body.length > 0 ? (
                                            <div className="group relative">
                                                <div className="flex items-center gap-1 cursor-help text-xs text-gray-500">
                                                    ...
                                                    <span className="truncate max-w-[100px]">{attack.post_body}</span>
                                                </div>
                                                {/* Tooltip for Body */}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 text-xs bg-white/5 rounded hover:bg-white/10 disabled:opacity-50">Prev</button>
                        <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 text-xs bg-white/5 rounded hover:bg-white/10 disabled:opacity-50">Next</button>
                    </div>
                </div>
            )}
        </div>
